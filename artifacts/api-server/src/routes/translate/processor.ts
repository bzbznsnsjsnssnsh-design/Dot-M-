...
const MAX_ATEMPO = 1.5;   // حد أقصى لسرعة الصوت — ما فوق 1.5 يُبطئ الفيديو بدلاً من تسريع الصوت
...
interface ProcessOptions {
  ...
  sourceLanguage?: string;   // ISO code e.g. "en", "fr". undefined = auto-detect
}
...
async function transcribeLocalWhisperJSON(
  ...
  // Build argument list: always include --json; add --lang only when specified
  const pyArgs = [whisperScriptPath, audioPath, "--json"];
...
async function processSentence(
  ...
  try {
    ...
    // ── نظام سرعة النطق الذكي ────────────────────────────────────────────────
    // إذا rawAtempo ≤ MAX_ATEMPO (1.5): يُشغَّل الصوت بالسرعة المطلوبة مباشرة
    // إذا rawAtempo > 1.5: يُثبَّت الصوت عند 1.5 + يُحسب معامل إبطاء الفيديو
    const clampedAtempo    = Math.min(MAX_ATEMPO, Math.max(MIN_ATEMPO, rawAtempo));
    // videoSlowRatio < 1.0 يعني الفيديو يحتاج للإبطاء للحفاظ على المزامنة
    const videoSlowRatio   = rawAtempo > MAX_ATEMPO
      ? MAX_ATEMPO / rawAtempo   // مثال: rawAtempo=2.0 → 1.5/2.0 = 0.75
      : 1.0;

    logger.info(
      { jobId, i, originalDuration, ttsDuration, rawAtempo, clampedAtempo, videoSlowRatio },
      "Sentence atempo"
    );
    ...
    if (j3?.sentences) {
      j3.sentences[i] = {
        ...
        videoSlowRatio,          // يُرسَل للواجهة لإبطاء الفيديو عند الحاجة
      };
      updateJob(jobId, {
...
export async function processVideoSegment(options: ProcessOptions): Promise<void> {
  ...
  try {
    ...
    const whisperSentences = await transcribeLocalWhisperJSON(audioInputPath, "small", sourceLanguage);
